...
for model in ${models}; do
       IDATE=`$NDATE +168 ${VDATE}00 | cut -c 1-8`
       echo "submitting jevs_${model}_severe_prep.sh for ${vhr}Z $VDATE cycle"
       qsub -v vhr=$vhr,VDATE=$VDATE,IDATE=$IDATE,OTLK_DATE=$OTLK_DATE,retro_name=$retro_name,COMINccpa=$COMINccpa,COMINobsproc=$COMINobsproc,DCOMINmrms=$DCOMINmrms,COMINnam=$COMINnam,COMINhrrr=$COMINhrrr,COMINhiresw=$COMINhiresw,COMINrrfs=$COMINrrfs ${RRFSevs}/dev/drivers/scripts/prep/cam/jevs_cam_${model}_severe_prep.sh
done
sleep 1
